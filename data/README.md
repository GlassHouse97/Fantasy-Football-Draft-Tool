...
- `warehouse/`: the local DuckDB database, including identity, participation, feature, target, and baseline-evaluation tables.
...
Historical entry-cohort rows require a weekly or participation position known before the cutoff. The current nflverse identity snapshot is not backfilled into those seasons, so 2,710 current-core historical candidate rows without cutoff-safe position evidence are excluded and reported. The August 2026 identity snapshot does predate the September 1, 2026 cutoff and safely supplies position for 309 live rows. Until a historical preseason-position archive is added, historical rookie baseline performance cannot be measured honestly. No statistical or machine-learning training has begun.
